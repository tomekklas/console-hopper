...
(async function () {
  ...
  // === DOM HELPER — minimal jQuery-subset shim ===
  // Console Hopper used to bundle jQuery 3.7.1 (85KB) but only ever called a
  // small, mechanical slice of its API. This shim implements exactly that slice
  // over the native DOM so the dependency can be dropped (see ROADMAP.md,
  // Stage 3). It is intentionally NOT general-purpose jQuery — only the methods
  // this file actually uses, with jQuery-faithful semantics where they differ
  // from the native equivalents (.data type-coercion, .show/.hide, delegation).
